...
一个使用原生 HTML5 Canvas + JavaScript 编写的现代风格贪吃蛇游戏。无需任何依赖、无需构建工具，**用浏览器打开 `index.html` 即可游玩**。

界面采用毛玻璃（Glassmorphism）质感设计，蛇身带渐变配色与发光食物，并对操作手感与渲染性能做了专门优化。

> 📦 本项目隶属于 [JYT 项目集合](../../README.md)，位于 `projects/snake/` 目录下。
...
开启 GitHub Pages 后，任何人都能直接在浏览器中游玩，无需下载：

**👉 https://jjeerry-ai.github.io/JYT/projects/snake/**

> 该链接在仓库开启 GitHub Pages 后约 1 分钟内生效。
...
cd JYT/projects/snake

# 直接用浏览器打开即可（任选其一）
#   Windows:  双击 index.html
#   或在终端: start index.html
```
...
```
projects/snake/
├── index.html          # 游戏本体（HTML + CSS + JS 全部在内）
├── docs/
...
打开 `index.html`，在 `<script>` 顶部可调整以下常量：
...
本项目基于仓库根目录的 [Apache-2.0](../../LICENSE) 许可证开源。
